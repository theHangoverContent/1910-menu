...
// Plate-realistic hotspot auto-layouts (client-side).
// Used for "first draft" hotspots + preview; backend has the authoritative generator too.
...
export function inferRole(ingredientId) {
...
function layoutCenterPerimeter({ ingredientIds, rng }) {
  ...
  for (let i = 0; i < core.length; i++) {
    ...
    pts.push({
      ingredientId: core[i].id,
      role: core[i].role,
      x: clamp01(0.52 + Math.cos(a) * r + jitter(rng, 0.05)),
      y: clamp01(0.58 + Math.sin(a) * r * 0.75 + jitter(rng, 0.05))
    });
  }
  for (let i = 0; i < rim.length; i++) {
    ...
    pts.push({
      ingredientId: rim[i].id,
      role: rim[i].role,
      x: clamp01(0.5 + Math.cos(ang) * rr + jitter(rng, 0.03)),
      y: clamp01(0.55 + Math.sin(ang) * rr + jitter(rng, 0.03))
    });
  }
  return relax(pts, { minDist: 0.06 });
}
...
function layoutRimOnly({ ingredientIds, rng }) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    pts.push({
      ingredientId: id,
      role,
      x: clamp01(0.50 + Math.cos(ang) * rr + jitter(rng, 0.02)),
      y: clamp01(0.56 + Math.sin(ang) * rr + jitter(rng, 0.02))
    });
  }
